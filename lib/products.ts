...
// Самая крупная настоящая розничная единица в каталоге (морозильник-сундук) — около
// 1.7 м³. Всё, что после конвертации litr×1000 превышает этот потолок, почти наверняка
// объём введён в Smartup в литрах, а не в м³ (напр. холодильник 712.5 «литр» — это и
// есть 712.5 л = 0.71 м³, а не 712.5 м³) — такие значения недостоверны для расчёта.
const MAX_PLAUSIBLE_VOLUME_L = 2500;
    ...
    const raw = (data.inventory || [])
      ...
      .map((i) => {
        ...
        return {
          ...
          // Но не у всех карточек так — у некоторых litr уже в литрах (см. MAX_PLAUSIBLE_VOLUME_L
          // выше), и ×1000 даёт фантастические тысячи литров. Такие отбрасываем как недостоверные
          // (volume_l=0) — товар получит приблизительный объём (медиана по группе) вместо неверного.
          weight: Number(i.weight_brutto) || Number(i.weight_netto) || 0,
          volume_l: ((Number(i.litr) || 0) * 1000) > MAX_PLAUSIBLE_VOLUME_L ? 0 : (Number(i.litr) || 0) * 1000,
        };
      })
      .filter((i) => i.code);
